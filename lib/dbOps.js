...
,   friendDiscover = require('../models/friendDiscoverData');


var dbOps = module.exports = {

    /**
     * trackUser() saves any profiles for which the current user hits "Track"
     * to the 'trackedUsers' collection.
     * @param req - request object coming from the client-side AJAX
     **/
        ...
        TrackedUser.find( { twitterId: q.twitterId }, function (err, user) {
            ...
            if (user.length === 0) {
                ...
                newUser.save(function (err, user) {
                    if (err) console.error('Error at MongoDB newUser.save(): ' + err);
     ...
     *
     **/
     dumpDiscoverData: function (handle, friends) {
         var now = new Date();
         var friendDiscoverData = new friendDiscover({
             //date: now,
             handle: handle,
             friends: friends
         });

         friendDiscoverData.update({ handle: handle }, { $addToSet: { friends: friendDiscoverData.friends } }, function (err, data) {
            if (err) throw new Error('Error at MongoDB friendDiscoverData.update(): ' + err);
