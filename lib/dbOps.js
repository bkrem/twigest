...
var dbOps = module.exports = {
    ...
    trackUser: function (req) {
        ...
        TrackedUser.find( { twitterId: q.twitterId }, function (err, user) {
            ...
            if (user.length === 0) {
                ...
                TrackedUser.update(
                    { twitterId: q.twitterId },
                    { $addToSet: { trackedBy: q.userhandle } },
                    function (err, u) {
    ...
    /**
     * Query the DB to find which profiles a given user of Twigest is following.
     *
     * @param {twigestUser} A string consisting of the Twigest user's Twitter handle.
     * @param {callback}    Passes returned data object to callback function.
     **/
    findTrackedUsers: function (twigestUser, callback) {
        TrackedUser.find( { trackedBy: twigestUser }, function (err, data) {
            if (err) throw new Error("Error at findTrackedUsers(): " + err);
            //console.log("findTrackedUsers() result:\n" + data);
            callback(data);
        });
    }
