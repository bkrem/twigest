...
TwigestAction.prototype.getFriendIds = function (context) {
	...
	// FIXME: Check for changes in submitted userhandle
...
TwigestAction.prototype.getFriendObjects = function (context) {
	...
	twigestCache.get('sessionIds', function (err, val) {
		...
		if (val !== undefined) {
			...
			self.twigest.get('friends/ids', { screen_name: context.userhandle, count: context.count || MAX_ID }, function (err, data) {
				if (err) {
					console.error('Twitter API GET for friends/ids encountered error: ' + err);
					callbackAndReturn(context.callback, err);
					return;
				}
...
TwigestAction.prototype.getStream = function (id) {
		this.twigest.get('statuses/user_timeline', { user_id: id, count: 200 }, function (err, data) {
			console.log(data);
		});
};
