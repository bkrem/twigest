	...
	twigestCache.get('sessionIds', function (err, val) {
		...
		if (val !== undefined) {
			...
			self.twigest.get('friends/ids', { screen_name: context.userhandle, count: context.count || MAX_ID }, function (err, data) {
				...
				twigestCache.set('sessionIds', data, function (err, success) {
					if (err) return console.error('twigestCache SET sessionIds failed: ' + err);
					console.log('twigestCache created key for sessionIds: ' + success);
				});
				ids = data.ids; // Array
				console.log(ids);
				callbackAndReturn(context.callback, ids);
...
TwigestAction.prototype.getFriendObjects = function (context) {
	...
	function splitIdsAndLookup(ids) {
