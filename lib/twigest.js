...
TwigestAction.prototype.getTechFriends = function (context) {
	var techKeywords = keywords.tech;
		...
		callback: function (friends) {
			...
			for (var i = 0; i < friends.length; i++) {
				for (var j = 0; j < techKeywords.length; j++) {
					if (friends[i].description.toLowerCase().indexOf(techKeywords[j]) !== -1 && friends[i].techLogged !== true) {
						techFriends.push(friends[i]);
						friends[i].techLogged = true;
						console.log(friends[i]);
					}
				}
			}
			console.log(techFriends);
			callbackAndReturn(context.callback, techFriends);
		}
	});
};
...
TwigestAction.prototype.getSportsFriends = function (context) {
	var sportsKeywords = keywords.sports;
	this.getFriendObjects({
		user: context.userhandle,
		count: context.count,

		callback: function (friends) {
			var sportsFriends = friends.filter(function (friend) {
				for (var i = 0; i < sportsKeywords.length; i++) {
					var isTechFriend = friend.description.toLowerCase().indexOf(sportsKeywords[i]) !== -1 ? friend : null;
					return isTechFriend;
				}
			});
			console.log(sportsFriends);
			callbackAndReturn(context.callback, sportsFriends);
		}
	});
};
